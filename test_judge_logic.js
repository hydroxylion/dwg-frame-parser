...
// k=1（1:1）不做尺寸误差判断：421×298 与 A3(420×297) 差 1mm，但非逐位相等 → 非标准
check('421x298 近 A3 差1mm 非标(1:1须精确)', 421, 298, 'fallback');
...
// 容差收紧到 2mm（2026-09-08，夹具装配图 420×597 被误判 A2 后收紧）：
//   842×2084 差 4mm 不再命中 k=1 的 A0+3/4（841×2080），改判 A2+3/4 × 2（k=2，2mm 内）——仍属标准缩放
check('842x2084 近似 A0+3/4 -> 改判 A2+3/4×2', 842, 2084, 'standard', 'A2+3/4 × 2');
// 420×597 ≈ A2(420×594) 差 3mm > 2mm：非标模板不得判标准（夹具装配图）
check('420x597 近 A2 差 3mm 非标模板', 420, 597, 'fallback');
check('420x600 近 A2 差 6mm 非标', 420, 600, 'fallback');
// 长中苑 202 室：自定义装修模板框，比例偏离 A 系 0.8%（>0.2%），缩放 39.05 非整数倍
// → 不得判"标准等比缩放"（曾误判 16 张 A2/A4 缩放 ~39.05x），应为非标准族
check('长中苑 23287x16333 非等比模板框', 23287, 16333, 'fallback', '非标准');
check('长中苑 11644x8166 非等比模板框', 11644, 8166, 'fallback', '非标准');
check('长中苑 封面 28261x19985', 28261, 19985, 'fallback', '非标准');
// 缩放 0.5 倍（比标准小）
